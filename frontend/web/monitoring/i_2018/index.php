...
//код карты
      ...
      if ($reg != "0" and $lg == "0")
      {
        ...
        echo '<head>';
    	echo '<title>map created with amCharts | amCharts</title>';
    	echo '<LINK href=style_light.css rel=stylesheet />';
	    echo '</head>';
	    echo '<body>';
	    echo '<p align=center><a class=Slova>';
            $pfd = "data/data.ini";
            $hat = File($pfd);
...
echo '</a></p>';
echo '<table align="center" width=820px;><tbody>';
echo '<tr height=30px; class=table_title>';
echo '<td align="center" valign="middle"><strong>Назва підприємства</strong></td>';
echo '<td align="center" valign="middle"><strong>Хвойні породи</strong></td>';
echo '<td align="center" valign="middle"><strong>Листяні породи</strong></td>';
echo '</tr>';
$gg=0;
            for ($les=0;$les<count($hyt);$les++)
                {   $hyt_ar = explode(";", $hyt[$les]);
                    ...
                    {
                        if ($gg == "0") {echo"<tr height=30px; class=table>";}
                        if ($gg == "1") {echo"<tr height=30px; class=table2>";}
                        echo '<td valign="middle">&nbsp';
                        echo "<a class=normal>$hyt_ar[1]</a>";
                        echo"</td>\n";
...
echo "</tbody></table>";
echo '<p style="text-align: center;"><a href=index.php class=link>повернутися до мапи</a><strong>';
	    echo '</body>';
        echo '</html>';
...
echo '<LINK href=style_light.css rel=stylesheet />';
echo "</head><body>";
	    echo '<p style="text-align: center;" class=Slova><strong>';
            $pfd = "data/data.ini";
            $hat = File($pfd);
            for ($fd=0;$fd<count($hat);$fd++)
                {
...
echo "$reg class=link>повернутися до списку.</a><strong>";
echo '<p style="text-align: center;"><a href=index.php class=link>повернутися до мапи</a><strong>';
echo "</body></html>";
            }
        if ($pd == 2)
